...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    ...
    try {
      await prisma.jadwal_detail.delete({
        where: { id: jadwaldetailId },
      });

      res.status(200).json({ message: "Detail jadwal deleted successfully" });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Error deleting detail jadwal" });
    }
  }
}
